...
GAME_CC_CMD = f"python3 tools/asm-processor/build.py {IDO_53_CC} -- {CROSS_AS} {AS_FLAGS} -- -G 0 -non_shared -fullwarn -woff 649,838,654 -verbose -Xcpluscomm -nostdinc -Wab,-r4300_mul $flags -mips2 {COMMON_INCLUDES} {IDO_DEFS} -c -o $out $in"
...
LIBULTRA_AS_CMD = f"{IDO_53_CC} -G 0 -non_shared -fullwarn -verbose -Wab,-r4300_mul -woff 513,516,649,838,712 $flags {COMMON_INCLUDES} -D_FINALROM -DBUILD_VERSION=VERSION_I -DLIBULTRA_DARK_RIFT -c -o $out $in && {O32_TOOL} $out && {CROSS_STRIP} $out -N asdasdasdasd"
...
def create_build_script(linker_entries: List[LinkerEntry]):
    ...
    ninja.rule(
        ...
        command=f"{CROSS}ld -T undefined_syms.txt -T undefined_syms_auto.txt -Map $mapfile -T $in -o $out",
    )
