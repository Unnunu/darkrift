...
u16 button_remap(u16 x_1b94c75c, u16 buttons) {
    u16 i;
    u16 mask;
    u16 v1;
    u16 *ptr = gRemapTable[x_1b94c75c];
...
void button_remap_reset(u16 x_1b94c75c) {
    s32 i;
    s32 mask;
    u16 *ptr = gRemapTable[x_1b94c75c];
...
#ifdef NON_MATCHING
void button_process(s16 x_1b94c75c) {
    s32 temp;
    u16 v1;
    u16 buttons;
    u16 v0;

    v1 = gContPadPtrs[x_1b94c75c]->button;
    buttons = sCButtonLut[v1 & 0xF];
    buttons |= sTriggerLut[(v1 >> 4) & 0xF];
    buttons |= sDPadLut[(v1 >> 8) & 0xF];
    buttons |= sFaceLut[(v1 >> 12) & 0xF];

    if (!(x_59ce598c + x_1b94c75c)->x_40ccbb76 && !(buttons & x_0684c746)) {
        if (gContPadPtrs[x_1b94c75c]->stick_x > 40) {
            buttons |= x_fc4a9735;
        } else if (gContPadPtrs[x_1b94c75c]->stick_x < -40) {
            buttons |= x_ad1aebe6;
        }

        if (gContPadPtrs[x_1b94c75c]->stick_y > 40) {
            buttons |= x_f7763ed8;
        } else if (gContPadPtrs[x_1b94c75c]->stick_y < -40) {
            buttons |= x_b8c91680;
        }
    }
    ...
    if ((x_59ce598c + x_1b94c75c)->x_3fb684fe) {
        buttons = button_remap(x_1b94c75c, buttons);
    }
    ...
    do { } while (0); }
#else
#pragma GLOBAL_ASM("asm/nonmatchings/buttons/button_process.s")
void button_process(s16 x_cc1d0de5);
#endif

void button_update_frame(void) {
    gFrameCounter++;
    controller_read_frame();

    if (x_59ce598c[x_83106b21].enabled && x_59ce598c[x_83106b21].x_12a7bd86) {
        button_process(x_83106b21);
    } else {
        x_59ce598c[x_83106b21].buttons = 0;
    }

    if (x_59ce598c[x_6f0b3be3].enabled && x_59ce598c[x_6f0b3be3].x_12a7bd86) {
        button_process(x_6f0b3be3);
    } else {
